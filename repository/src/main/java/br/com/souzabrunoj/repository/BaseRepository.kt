...
import br.com.souzabrunoj.data.service.NetworkingError
...
abstract class BaseRepository {
    protected suspend fun <R : Any?> doRequest(block: suspend () -> Either<Failure.ServiceError, R>) =
        try {
            ...
            when (error) {
                is NetworkingError -> Error(
                    Failure.ServiceError(
                        error.errorDescription ?: error.message, httpCode = error.httpCode
                    )
                )
                else -> Error(
                    Failure.ServiceError(messageInternal = error.message, httpCode = (error as? HttpException)?.code())
                )
            }
        }
